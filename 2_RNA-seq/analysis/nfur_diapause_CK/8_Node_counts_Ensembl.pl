...
my $infile = '4_Paralog_classification/4_Ensembl'; 
my $outfile = '5_Node_counts/4_Ensembl';           
...
'Nothobranchius furzeri (lineage specific)');
...
# Killifish nodes
my %Killifish_nodes = ('Atherinomorphae' => '',
'Aplocheiloidei' => '',
'Nothobranchiadae' => '',
'Nothobranchius furzeri' => '',
'Nothobranchius furzeri (lineage specific)' => '');
...
#	if ($line[14] == 1 && $line[9] >= 50){ # With gene only duplicated once, less genes ~1200
		...
		if (exists $Vertebrate_nodes{$line[2]}){
			...
			if ($line[15] eq $category){$AgeDiffCounts{'Vertebrates'} += 1;}
		}
		if (exists $Fish_nodes{$line[2]}){
			$AgeCounts{'Fish'} += 1;
			if ($line[15] eq $category){$AgeDiffCounts{'Fish'} += 1;}
		}
		if (exists $Killifish_nodes{$line[2]}){
			$AgeCounts{'Killifish'} += 1;
			if ($line[15] eq $category){$AgeDiffCounts{'Killifish'} += 1;}
		}
