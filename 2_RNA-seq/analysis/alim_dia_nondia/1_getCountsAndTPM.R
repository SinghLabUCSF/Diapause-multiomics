...
# Input variables ---------------------------
genename = "gene_name"
gtffile = "../../Datasets/Alim_genome/ref_Austrofundulus_limnaeus-1.0_top_level.gtf"
outfilepath = "../../feature_counts"
sampleTable <- read.csv("../../fastq/alim_dia_nondia/experiment_design_D-nonD.csv", row.names = 1, stringsAsFactors = F)
# ------------------------------------------
sampleTable$files
...
for (i in 1:length(sampleTable$files)){
  ...
  outfilename
  cts = featureCounts(sampleTable$files[i], annot.ext = gtffile, isGTFAnnotationFile = T, GTF.attrType = genename, isPairedEnd = T)
