...
sampleTable <- read.csv("../../../fastq/drer/experimental_design_zebrafish.csv", row.names = 1)
sampleTable

library("DESeq2")

countdata = read.csv("Counts_zebrafish_development_all.csv", row.names = 1)
colnames(countdata)
coldata <- DataFrame(sampleTable)
rownames(coldata)
colnames(countdata) == rownames(coldata) # To make sure that the column names are identical. Make sure they correspond to each other even if the names are not identical
...
write.table(normcounts, file="CountsNormDESeq2_zebrafish_development_all.csv", sep= ",")
head(normcounts)
