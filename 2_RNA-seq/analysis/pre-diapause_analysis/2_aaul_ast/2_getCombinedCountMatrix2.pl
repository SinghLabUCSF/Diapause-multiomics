...
## -------------------------------------------------------------------------------------------------------------------
my $designfile = '../../../RNA-seq/fastq/nfur_ast_aaul/experiment_design_aaul-ast_young_KV.csv';
my $tpmfile = '../../../feature_counts';
my $outct = 'Counts_development_aaul-ast_young_KV.csv';
#my $outtpm = 'TPM_nfur_diapause_aaul-ast.csv';
#my $outlen = 'GeneLengths_nfur_diapause_aaul-ast.csv';
## -------------------------------------------------------------------------------------------------------------------
...
foreach (@libOrder){
	if ($_ eq $lastlib){ # If last library print without comma
		print CT "\"$sampleNames{$_}\""; 
		#print TPM "\"$sampleNames{$_}\""; print LEN "\"$sampleNames{$_}\"";
	}
	else {
		print CT "\"$sampleNames{$_}\","; 
		#print TPM "\"$sampleNames{$_}\","; print LEN "\"$sampleNames{$_}\",";
