# Filter all paralog pairs to remove redundancy and multi-gene families and get a list for further analysis.
# The 3 lists highlighted as LIST 1,2,3 were used for my analysis.
use strict;
use warnings;
use List::Util qw( min max );

# Outfiles from most relaxed to most stringent criteria.

# All: 46201 pairs

# All pairs after filtering more than 20 paralog partners 
open R0, ">nfur/List_All_20paralogs.txt"; # 20091 *************************************************** LIST 1
...
open I1, ">nfur/List_Intermediate_1.txt"; # ********************************************************* LIST 2
# Highest similarity in family with >=50% similarity (7390 pairs) 
#open I2, ">List_Intermediate_2.txt";

=cut
# Highest similarity in family non-redundant (6149 pairs)
open I3, ">List_Intermediate_3.txt";
# Highest similarity in family non-redundant >50% similarity (4435 pairs)
open I4, ">List_Intermediate_4.txt";
=cut 

# Only duplicated once (2228 pairs) ************************************************************* LIST 3
open S1, ">nfur/List_Strict_1.txt";
# Only duplicated once with >50% similarity (1470 pairs)
#open S2, ">List_Strict_2.txt";

open FH, "nfur/nfur_paralogs_KaKs_71spp_highestSimilarity.txt" or die $!;
my @file = <FH>;
my $head = shift @file;

print R0 "$head"; print I1 "$head"; print S1 "$head";
#print R1 "$head"; print R2 "$head"; print R3 "$head";
#print I2 "$head"; print I3 "$head"; print I4 "$head"; 
#print S2 "$head";  print R0A "$head";
...
# Read all duplicate file and make 2 hashes: one for All pairs and one with only similarity in both directions
foreach (@file){
